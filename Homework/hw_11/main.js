// TASK 1_______________________________
   ...
   .then(value => value.json())
    .then(cartsObject => {
        ...
        for(const cart of carts){
            ...
            div.innerText = `${cart.id} ${cart.title} ${cart.price} ${cart.quantity} ${cart.total} ${cart.discountPercentage} ${cart.discountedTotal} ${cart.thumbnail}`;
            cartsDiv.appendChild(div);
        }

        // запис інформації стрінгою

    });

// II варіант - складний
        // Створемо під кожну характеристику блок

const cartsDiv2 = document.getElementById('carts2');
fetch('https://dummyjson.com/carts')
    .then(value => value.json())
    .then(cartsObject => {
        const {carts} = cartsObject;
        ...
        for(const cart of carts){
            ...
            div2.classList.add('cart-container');
            ...
            // Щоб було цікавіше, ускладнемо це списком продуктів, який є в середині
            ...
            div2.append(divWithInfo, ol);
            cartsDiv2.appendChild(div2);
        }
